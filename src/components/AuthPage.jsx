      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-xl">
          ...
          <div className="glass rounded-[1.5rem]" style={{ padding: '1.5rem', marginBottom: '1.5rem' }}>
            ...
            {/* Token type toggle */}
            <div className="flex gap-2 rounded-xl" style={{ background: 'rgba(0,0,0,0.3)', padding: '0.25rem', marginBottom: '1rem' }}>
              {[
                ...
                <button
                  ...
                  style={tokenType === id
                    ? { padding: '0.5rem 0.75rem', background: 'rgba(192,193,255,0.15)', color: '#c0c1ff', border: '1px solid rgba(192,193,255,0.3)' }
                    : { padding: '0.5rem 0.75rem', background: 'transparent', color: '#908fa0', border: '1px solid transparent' }
                  }
                >
                  {label}
                </button>
            ...
            {tokenType === 'classic' && (
              <div className="flex items-start gap-2 rounded-xl text-xs"
                style={{ padding: '0.625rem 0.75rem', marginBottom: '1rem', background: 'rgba(255,183,131,0.08)', border: '1px solid rgba(255,183,131,0.25)', color: '#ffb783' }}>
            ...
            {/* Permissions table */}
            <div style={{ marginTop: '1rem', borderRadius: '0.75rem', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)' }}>
              <table className="w-full text-xs">
                <thead>
                  <tr style={{ background: 'rgba(192,193,255,0.08)' }}>
                    <th style={{ padding: '0.625rem 1rem', textAlign: 'left' }} className="text-[#c0c1ff] font-semibold">
                      {tokenType === 'finegrained' ? 'Permiso' : 'Scope'}
                    </th>
                    <th style={{ padding: '0.625rem 1rem', textAlign: 'left' }} className="text-[#c0c1ff] font-semibold">Acceso</th>
                  </tr>
                </thead>
                <tbody style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                  {(tokenType === 'finegrained' ? PERMS_FINEGRAINED : PERMS_CLASSIC).map(([p, a]) => (
                    <tr key={p} style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                      <td style={{ padding: '0.625rem 1rem' }} className="text-[#c7c4d7] font-mono">{p}</td>
                      <td style={{ padding: '0.625rem 1rem' }}>
                        <span
                          style={{ padding: '0.25rem 0.625rem', borderRadius: '9999px' }}
                          className="text-xs"
                          style={{
          ...
          {/* Token form */}
          <form onSubmit={handleSubmit} className="glass rounded-[1.5rem]" style={{ padding: '1.5rem', marginTop: '4rem' }}>
            ...
            <div className="relative" style={{ marginBottom: '1.25rem' }}>
              <input
                ...
                className="w-full rounded-xl font-mono outline-none transition-all duration-200"
                style={{
            ...
            {error && (
              <div
                ...
                style={{ padding: '0.75rem 1rem', marginBottom: '1.25rem', background: 'rgba(242,139,130,0.1)', border: '1px solid rgba(242,139,130,0.3)', color: '#f28b82' }}
              >
